...
3. Open any item to read its redacted content, scope, source and resolution reason.
4. Identify duplicate definitions, ambiguous names and unknown states.
5. Open the original file in the editor when a change is needed.
...
- macOS first, local-only and read-only.
